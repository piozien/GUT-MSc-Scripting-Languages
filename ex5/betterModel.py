...
class GlassNet(nn.Module):
    ...
    def forward(self, x):
        x = self.fc1(x)
        x = self.relu(x)
        x = self.dropout(x)  # Dropout po 1 warstwie
        x = self.fc2(x)
        x = self.relu(x)
        x = self.fc3(x)
        return x
...
for epoch in range(epochs):
    model.train() # Dropout na czas uczenia
    ...
    if (epoch + 1) % 50 == 0:
        ...
        with torch.no_grad():
            ...
            print(f'Epoka [{epoch + 1}/{epochs}], Loss: {loss.item():.4f}, Val Loss: {val_loss.item():.4f}, Val Acc: {accuracy:.2f}%')
